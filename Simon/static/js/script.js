...
function touches_partie(i)
{
    ...
    if (niveau_int > 2)
    {
        let temps_chg = 500;    //c'est une partie de niveau 2 alors ça va deux fois plus vite
...
function niveau() 
{
    var to_put = "/topniveau?level="+document.getElementById('sN').value; 
    loadDoc(to_put, myFunction);
}


function son_jouer(liste)
{   
    const elem_sound = liste[Math.floor(Math.random() * liste.length)];
    var sound = document.getElementById(elem_sound);
    sound.play();
}